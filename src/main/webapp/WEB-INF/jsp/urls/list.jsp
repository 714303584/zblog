	...
	<div id="div_table_main" class="panel panel-default"
		...
		<div id="div_table_heading" class="panel-heading"
			...
			<ol class="breadcrumb" style="margin: 0px">
				<li><a href="/admin/urls/list.html">需授权路径管理</a></li>
				<li><a href="/admin/urls/list.html">需授权路径列表</a></li>
			</ol>
		</div>
		<div id="div_table_controller" class="center" style="padding: 4px;">
			<a class="btn btn-primary btn-xs" href="/admin/urls/add.html"
				target="frane_two">添加</a> <a id="deleteByIds" class="btn btn-primary btn-xs">删除</a>
		</div>
		
		<div style="display: none;">
			<form id="deleteForm" action="/admin/urls/deleteByIds" method="post">
			...
			<div id="table_databody">
				<table class="table  table-hover table-condensed b-t text-sm"
					...
					<tbody id="table_body" style="padding: 0px;">
						 ...
						 <c:forEach var="urls" items="${pages.elements}">
							<tr>
								<td><input type="checkbox"name="post[]" value="${urls.id}">${urls.id}</td>
								<td>${urls.name }</td>
								<td>${urls.pname }</td>
								<td>${urls.url }</td>
								<td><a href="/admin/urls/show.html?id=${urls.id}" class="btn btn-primary btn-xs">编辑</a> <a
									href="/admin/urls/delete.html?id=${urls.id}"
