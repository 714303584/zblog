...
.controller('MainController', function(
    ...
    function loadPexRTC(host) {
        ...
        pexrtcScript.src = $sce.trustAsResourceUrl('http://' + host + '/static/webrtc/js/pexrtc.js');
    ...
    $scope.loginApp = function(alias, displayName, skipDeviceSelectionDialog) {
        ...
        .then(function(invalidDevices) {
            angular.forEach(invalidDevices, function(invalidDevice) {
                ...
                $scope.getUserMediaError = 'IDS_ESCALATE_INVALID_DEVICE';
            });
    ...
    $scope.disconnect = function(navigate) {
        ...
        if ($scope.presentationWindow) {
            $scope.presentationWindow.close(true);
            delete $scope.presentationWindow;
        }
    ...
    $scope.getConferenceAvatarUrl = function(alias, host) {
        return 'http://' + (host || document.domain) + '/api/client/v2/conferences/' + alias + '/avatar.jpg';
    };
    ...
    $scope.$on('call::remoteMediaStream', function(event, stream, type) {
        ...
        switch (type) {
            case 'video':
                ...
                if ($localStorage.audioOutputId) {
                    $timeout(function() {
                        document.querySelector('video.sel-main-video').setSinkId($localStorage.audioOutputId);
                    });
                }
                break;
            case 'rtmp':
    ...
    function closePresentationWindowOnEnd(event) {
        if ($scope.presentationWindow) {
            $scope.presentationWindow.close();
    ...
    $scope.popOutPresentation = function() {
        if (!$scope.presentationWindow) {
            if (platformSettings.isDesktopClient) {
                $scope.presentationWindow = nw.Window.open(
                    '../templates/presentation-window.html', {
                        ...
                        $scope.$apply(function() {
                            $scope.presentationWindow = win;
                        });
                        win.on('close', function(event) {
                            $timeout(function() {
                                $scope.presentationWindow.close(true);
                                delete $scope.presentationWindow;
                                $scope.call.getPresentationUrl();
                            });
                        });
                        win.once('loaded', function() {
                            $timeout(function() {
                                ...
                                $scope.presentationWindow.show();
                                $scope.presentationWindow.focus();
                            });
                        });
                    });

            } else {
                $scope.presentationWindow = window.open(
                    'templates/presentation-window.html',
                ...
                $scope.presentationWindow.onbeforeunload = function() {
                    $timeout(function() {
                        delete $scope.presentationWindow;
                        $scope.call.getPresentationUrl();
                    });
                };
                $scope.presentationWindow.onload = function() {
                    $timeout(function() {
                        $scope.call.getPresentationUrl();
                        $rootScope.$broadcast(
                            'call::presentationVideoUpdate',
